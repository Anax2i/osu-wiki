---
stub: true
tags:
  - easy
  - EZ
  - mod
  - game modifier
  - lazer
---

# Easy (lazer mod)

![EZ mod icon](/wiki/Gameplay/Game_modifier/Lazer/Mods/EZ.png "Easy (EZ) mod icon")

*For the full list of all lazer mods, see: [Game modifier (lazer)](/wiki/Gameplay/Game_modifier/Lazer)*

*For the stable wiki, see: [Easy (stable)](/wiki/Gameplay/Game_modifier/Easy)*

## About

- Abbreviation: EZ
- Type: Difficulty Reduction
- Score Multiplier: 0.50x
- Default shortcut key: `Q`
- Caption:
  - ![osu!]: `Larger circles, more forgiving HP drain, less accuracy required, and three lives!`
  - ![osu!taiko]: `Beats move slower, and less accuracy required!`
  - ![osu!catch]: `Larger fruits, more forgiving HP drain, less accuracy required, and three lives!`
  - ![osu!mania]: `More forgiving HP drain, less accuracy, and three lives!`
- Compatible game modes: ![][osu!] ![][osu!taiko] ![][osu!catch] ![][osu!mania]
- Incompatible mods: [Accuracy Challenge (AC)](/wiki/Gameplay/Game_modifier/Lazer/Accuracy_Challenge), [Hard Rock (HR)](/wiki/Gameplay/Game_modifier/Lazer/Hard_Rock), [Difficulty Adjust (DA)](/wiki/Gameplay/Game_modifier/Lazer/Difficulty_Adjust)
- Status: **RANKED**

## Gameplay

![](/wiki/Gameplay/Game_modifier/Lazer/Mods/Gameplay/easy.gif)

Comparison between a regular play (right) vs a play with the Easy mod enabled (left) in osu!

*Beatmap: [dark cat - BUBBLE TEA (feat. juu & cinders) (Cut Ver.) [Ag's Hard]](https://osu.ppy.sh/beatmapsets/2215155#osu/4696370)*

[osu!]: /wiki/Gameplay/Game_modifier/Lazer/Logo/osu!.png "osu!"
[osu!taiko]: /wiki/Gameplay/Game_modifier/Lazer/Logo/osu!taiko.png "osu!taiko"
[osu!catch]: /wiki/Gameplay/Game_modifier/Lazer/Logo/osu!catch.png "osu!catch"
[osu!mania]: /wiki/Gameplay/Game_modifier/Lazer/Logo/osu!mania.png "osu!mania"
